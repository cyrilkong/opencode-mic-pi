...
node bin/opencode-router.js build-model-evidence --source-spec scripts/evidence-sources.example.yaml --audit-path "$HOME/.local/share/opencode/plugins/opencode-router/global/model-discovery-audit.json"
node scripts/build-model-evidence.mjs --source-spec scripts/evidence-sources.example.yaml --audit-path "$HOME/.local/share/opencode/plugins/opencode-router/global/model-discovery-audit.json" --out defaults/evidence
npm pack --dry-run
```
...
- evidence-backed model routing: drop a snapshot-bound multi-source `model-evidence.<shortFingerprint>.json` next to the verified discovery audit (path precedence: `evidence_catalog_path` -> `evidence_catalog_glob` -> `OPENCODE_ROUTER_EVIDENCE_JSON` -> `OPENCODE_ROUTER_EVIDENCE_DIR` -> bundled `defaults/evidence/`); raise `evidence_rank_strength` (default `0`) so fused SWE / IQ-style scores drive the rank dimensions while `coding` / `reasoning` token-name dimensions contribute zero. Naming is never a rank input; provider/keyword/family policy stays for **exclusion only**.
- evidence bundles must carry a `pool_fingerprint` matching the current `model-discovery-audit.json`; fingerprint mismatch surfaces a warning, applies neutral evidence, and **never** falls back to name-token rank.
- maintainers build evidence bundles offline with `node scripts/build-model-evidence.mjs --source-spec <yaml> --audit-path <discovery-audit.json> --out defaults/evidence`; sources can be `local-json`, `local-yaml`, `inline`, or deliberate `url` fetch. See `defaults/evidence/README.md` and `scripts/evidence-sources.example.yaml`.
- `recommendation.evidence_catalog` (top-level) and per-role `evidence_hit` / `evidence_basis` / `evidence_fusion` / `evidence_by_source` / `pool_fingerprint_match` give auditable transparency for ranking decisions.
- optional **web-grounded model research** (`model_research_enabled`): before persisting `model-match`, the plugin can spawn `scripts/run-model-research-runner.mjs` (override with `model_research_runner_path`), validate authority-tier citations against `defaults/research-authority-allowlist.json`, then write `projects/<stable-project-key>/model-research.json`; ranking blends that signal **7:3** with markdown policy soft scores after hard avoids (`global_avoid_keywords` / policy keyword and family avoids). The default runner exits unsuccessfully unless `OPENCODE_ROUTER_RESEARCH_MOCK=1` (tests) or `OPENCODE_ROUTER_ALLOW_STUB_WEB_TOOLS=1` (local stub); wire a real OpenCode-backed runner for production web tools. See `docs/agent-model-match.md` §3.5.3.
...
- canonical plugin state is split between app-data `global/` (`model-match`, `model-discovery-audit`) and app-data `projects/<stable-project-key>/` (intake, dispatch, workboard, session-language, interaction-mode, relay-bridge, research-memory, memory-palace, `model-research.json`, and other memory-palace artifacts)
